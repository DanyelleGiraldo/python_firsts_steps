...
def actpaloct(letra, palabra, palocult):
    for i in range(len(palabra)):
        if palabra[i] == letra:
            palocult[i] = letra  # Corregido: asignación en lugar de uso de ":"
...
def jugaho():
    ...
    while intentos_restantes > 0 and equivocado(palabra_oculta, palabra):  # Corregido: cambio de intrest a intentos_restantes
    ...
    if not equivocado(palabra_oculta, palabra):
